...
class SessionState:
    def __init__(self) -> None:
        ...
        self._cloud_sync_delay_seconds: float = 2.5
...
def ensure_ladder_scene_state(self, new_visit: bool = False, allow_cached: bool = False) -> None:
    ...
    cache_ok = bool(
        allow_cached
        and not new_visit
        and bool(self.public_ladder_rows_cache)
        and bool(getattr(self, 'public_pq_ledger_rows_cache', []))
...
def refresh_guild_hall_state(self, allow_cached: bool = False, force: bool = False) -> bool:
    guild_row = getattr(self, 'guild_row', {}) if isinstance(getattr(self, 'guild_row', {}), dict) else {}
    guild_invite_rows = getattr(self, 'guild_invite_rows', []) if isinstance(getattr(self, 'guild_invite_rows', []), list) else []
    guild_member_rows = getattr(self, 'guild_member_rows', []) if isinstance(getattr(self, 'guild_member_rows', []), list) else []
    if allow_cached and not force and (guild_row or guild_invite_rows or guild_member_rows) and hasattr(self, '_scene_refresh_stale') and not self._scene_refresh_stale('guild_hall'):
        return True
    self.guild_row = {}
...
def refresh_guild_leaderboard(self, force: bool = False, allow_cached: bool = False) -> bool:
    if allow_cached and not force and self.guild_leaderboard_rows and hasattr(self, '_scene_refresh_stale') and not self._scene_refresh_stale('guild_leaderboard'):
        return True
...
def refresh_bazaar_listings(self, force: bool = False, allow_cached: bool = False) -> None:
    ...
    if not force and self.bazaar_listings and (time.time() - self.bazaar_last_refresh_at) < 8.0:
        return
    if allow_cached and not force and self.bazaar_listings and hasattr(self, '_scene_refresh_stale') and not self._scene_refresh_stale('bazaar'):
        return
