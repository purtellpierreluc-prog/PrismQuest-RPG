...
class SessionState:
    ...
    def ensure_auth_session(self) -> bool:
        if self.supabase is None:
            ...
            return False
        known_user_id = str(getattr(self, 'auth_user_id', '') or '')
        known_user_email = str(getattr(self, 'auth_user_email', '') or '')
        ...
        for slot_index, raw_slot in enumerate(self.slots, start=1):
            ...
            if profile_payload is not None:
                ...
                if force or self._last_public_profile_hashes.get(mode) != profile_hash:
                    profile_payloads = [profile_payload]
                    if 'inventory_items' in profile_payload:
                        legacy_profile_payload = dict(profile_payload)
                        legacy_profile_payload.pop('inventory_items', None)
                        profile_payloads.append(legacy_profile_payload)
                    for candidate_payload in profile_payloads:
                        try:
