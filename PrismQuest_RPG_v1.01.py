...
MONSTER_ARCHETYPES = [
    ...
    {"type": "Black Joker", "school": "magic", "kit": "Shadow rags + cursed cards", "visual": "black_joker", "profile": "Jester hexer: wild spikes, nimble frame, thin guard", "hp_mult": 0.82, "damage_mult": 1.08, "variance_mult": 2.25, "phys_mult": 0.35, "mres_mult": 1.35, "speed_bonus": 4, "accuracy_bonus": 0.03, "crit_bonus": 0.06, "evasion_bonus": 0.10},
    {"type": "Black Knight", "school": "physical", "kit": "Black plate + horned blade", "visual": "black_knight", "profile": "Grave duelist: heavy guard, measured swings, stubborn ward", "hp_mult": 1.22, "damage_mult": 0.98, "variance_mult": 0.70, "phys_mult": 1.85, "mres_mult": 1.05, "speed_bonus": -2, "accuracy_bonus": 0.01, "crit_bonus": 0.02, "evasion_bonus": 0.01},
    {"type": "Fairy", "school": "magic", "kit": "Gossamer wings + dusk charm", "visual": "fairy", "profile": "Gleam trickster: frail, elusive, spell-bright bursts", "hp_mult": 0.72, "damage_mult": 1.02, "variance_mult": 1.70, "phys_mult": 0.25, "mres_mult": 1.80, "speed_bonus": 6, "accuracy_bonus": 0.06, "crit_bonus": 0.04, "evasion_bonus": 0.16},
    {"type": "Pixie", "school": "magic", "kit": "Needle wand + stolen starlight", "visual": "pixie", "profile": "Tiny menace: extreme evasion, precise sparks, paper shell", "hp_mult": 0.68, "damage_mult": 0.98, "variance_mult": 1.95, "phys_mult": 0.20, "mres_mult": 1.65, "speed_bonus": 8, "accuracy_bonus": 0.07, "crit_bonus": 0.05, "evasion_bonus": 0.18},
    {"type": "Snekers", "school": "physical", "kit": "Hooked claws + venom grin", "visual": "snekers", "profile": "Serpent brawler: quick coils, mean crits, balanced shell", "hp_mult": 0.98, "damage_mult": 1.06, "variance_mult": 1.50, "phys_mult": 0.80, "mres_mult": 0.80, "speed_bonus": 4, "accuracy_bonus": 0.04, "crit_bonus": 0.03, "evasion_bonus": 0.08},
...
MONSTER_DIALOGUE = {
    ...
    'Black Joker': ['The joke is funnier if you survive long enough to hear the end.', 'Pick a card. No, not that one. That one bites.'],
    'Black Knight': ['The visor closes. The verdict was already inside it.', 'Steel does not need to hurry when fear walks toward it.'],
    'Fairy': ['Tiny wings. Terrible manners. Perfect aim.', 'Careful. The sparkles are the part that hurts.'],
    'Pixie': ['I am small enough to miss and mean enough to matter.', 'Blink twice if you want me to move somewhere worse.'],
    'Snekers': ['The hiss is not a warning. It is applause.', 'Step closer. I promise the floor only moved once.'],
...
MONSTER_THEME_MAP = {
    ...
    'Black Joker': {'rgb': '244, 244, 245', 'shell': 'rgba(22, 22, 28, 0.94)', 'deep': 'rgba(7, 7, 11, 0.99)'},
    'Black Knight': {'rgb': '156, 163, 175', 'shell': 'rgba(23, 24, 29, 0.94)', 'deep': 'rgba(8, 9, 12, 0.99)'},
    'Fairy': {'rgb': '190, 242, 100', 'shell': 'rgba(21, 35, 18, 0.94)', 'deep': 'rgba(8, 15, 9, 0.99)'},
    'Pixie': {'rgb': '94, 234, 212', 'shell': 'rgba(12, 33, 36, 0.94)', 'deep': 'rgba(7, 15, 17, 0.99)'},
    'Snekers': {'rgb': '132, 204, 22', 'shell': 'rgba(22, 32, 14, 0.94)', 'deep': 'rgba(9, 14, 8, 0.99)'},
...
_TITLE_SCREEN_ASSET_META_CACHE: Dict[str, Tuple[str, int, int]] = {}
...
def get_title_screen_asset_meta() -> Tuple[str, int, int]:
    ...
    if cached is not None:
        return cached
    meta = (_image_path_to_data_uri(path, crop_alpha=False), *_get_image_dimensions(path))
    _TITLE_SCREEN_ASSET_META_CACHE.clear()
    _TITLE_SCREEN_ASSET_META_CACHE[cache_key] = meta
    return meta
...
AUTH_RECONNECT_GRACE_SECONDS = 15.0
...
def _chat_cutoff_datetime(now: Optional[datetime] = None) -> datetime:
    ...
    return base.astimezone(timezone.utc) - timedelta(days=CHAT_MESSAGE_RETENTION_DAYS)


def _chat_cutoff_iso(now: Optional[datetime] = None) -> str:
    return _chat_cutoff_datetime(now).replace(microsecond=0).isoformat().replace('+00:00', 'Z')


def _chat_created_at_datetime(value: object) -> Optional[datetime]:
    raw = str(value or '').strip()
    if not raw:
        return None
...
def _chat_message_is_recent(message: Dict[str, str], cutoff: Optional[datetime] = None) -> bool:
    created_at = _chat_created_at_datetime(message.get('created_at', ''))
    if created_at is None:
        return True
    return created_at >= (cutoff or _chat_cutoff_datetime())


def _chat_message_sort_key(message: Dict[str, str]) -> Tuple[float, str]:
    created_at = _chat_created_at_datetime(message.get('created_at', ''))
    timestamp = created_at.timestamp() if created_at is not None else 0.0
    return (timestamp, str(message.get('id') or ''))


def _recent_chat_messages(messages: List[Dict[str, str]], limit: int) -> List[Dict[str, str]]:
    cutoff = _chat_cutoff_datetime()
    visible = [message for message in messages if _chat_message_is_recent(message, cutoff)]
    visible.sort(key=_chat_message_sort_key, reverse=True)
...
MONSTER_ASSET_FILENAMES = {
    ...
    'Black Joker': ['Black_Joker.png', 'Black Joker.png'],
    'Black Knight': ['Black_Knight.png', 'Black Knight.png'],
    'Fairy': ['Fairy.png'],
    'Pixie': ['Pixie.png'],
    'Snekers': ['Snekers.png'],
...
class SessionState:
    def __init__(self) -> None:
        ...
        self.guild_row: Dict[str, object] = {}
        self.guild_membership_row: Dict[str, object] = {}
        self.guild_member_rows: List[Dict[str, object]] = []
        self.guild_invite_rows: List[Dict[str, object]] = []
        self.guild_status: str = 'The hall is quiet.'
        self.guild_status_tone: str = 'info'
        self.guild_leaderboard_rows: List[Dict[str, object]] = []
        ...
        self._cloud_sync_delay_seconds: float = 3.5
        self._last_cloud_sync_request_at: float = 0.0
        self._last_cloud_sync_flush_at: float = 0.0
        self._public_sync_min_interval_seconds: float = 15.0
        self._last_public_sync_at: float = 0.0
        self._public_sync_dirty: bool = False
        self._auth_reconnect_started_at: float = 0.0
        self._auth_reconnect_grace_until: float = 0.0
        self._auth_reconnect_known_user_id: str = ''
        self._auth_reconnect_known_user_email: str = ''
        self._auth_reconnect_last_error: str = ''
        self._last_runtime_session_hash: str = ''
        self._scene_refresh_timestamps: Dict[str, float] = {}
        self._scene_refresh_ttls: Dict[str, float] = {
            'ladder': 15.0,
            'guild_leaderboard': 15.0,
            'guild_hall': 15.0,
            'bazaar': 15.0,
            'marketplace': 20.0,
        }
    ...
    def _stored_auth_session(self) -> Dict[str, str]:
        ...
        return {
            ...
            'user_email': str(stored.get('user_email') or ''),
        }
        ...
        try:
            ...
            if access_token and refresh_token:
                app.storage.user['supabase_session'] = {
                    'access_token': access_token,
                    'refresh_token': refresh_token,
                    'user_id': str(getattr(self, 'auth_user_id', '') or ''),
                    'user_email': str(getattr(self, 'auth_user_email', '') or ''),
                    'saved_at': time.time(),
                }
        except Exception:
            pass
    ...
    def _auth_reconnect_remaining_seconds(self) -> float:
        until = float(getattr(self, '_auth_reconnect_grace_until', 0.0) or 0.0)
        if until <= 0.0:
            return 0.0
        return max(0.0, until - time.monotonic())

    def _auth_reconnect_active(self) -> bool:
    ...
    def _begin_auth_reconnect_grace(self, user_id: str = '', user_email: str = '', error_text: str = '') -> bool:
        ...
        backup = self._stored_cloud_slot_backup()
        known_user_id = (
            str(user_id or '').strip()
            or str(getattr(self, 'auth_user_id', '') or '').strip()
            or str(tokens.get('user_id') or '').strip()
            or str(backup.get('user_id') or '').strip()
            or str(getattr(self, '_auth_reconnect_known_user_id', '') or '').strip()
        )
        known_user_email = (
            str(user_email or '').strip()
            or str(getattr(self, 'auth_user_email', '') or '').strip()
            or str(tokens.get('user_email') or '').strip()
        ...
        self._auth_reconnect_known_user_email = known_user_email
        self._auth_reconnect_last_error = str(error_text or '')[:180]
        self.auth_user_id = known_user_id
        if known_user_email:
    ...
    def ensure_auth_session(self) -> bool:
        ...
        backup = self._stored_cloud_slot_backup()
        known_user_id = (
            str(getattr(self, 'auth_user_id', '') or '')
            or str(getattr(self, '_auth_reconnect_known_user_id', '') or '')
            or str(tokens.get('user_id') or '')
            or str(backup.get('user_id') or '')
        )
        known_user_email = (
            str(getattr(self, 'auth_user_email', '') or '')
            or str(getattr(self, '_auth_reconnect_known_user_email', '') or '')
            or str(tokens.get('user_email') or '')
        )

        def restore_known_user() -> None:
            if known_user_id and not str(getattr(self, 'auth_user_id', '') or ''):
                self.auth_user_id = known_user_id
            if known_user_email and not str(getattr(self, 'auth_user_email', '') or ''):
                self.auth_user_email = known_user_email
        ...
        for attempt in range(2):
            if self._refresh_auth_session(refresh_token):
                self._clear_auth_reconnect_grace()
                return True
            restore_known_user()
            if self.refresh_authenticated_user(persist_session=False):
        ...
        return self._begin_auth_reconnect_grace(known_user_id, known_user_email)
    ...
    async def _run_queued_cloud_sync(self) -> None:
        try:
            while True:
                ...
                if (
                    bool(getattr(self, 'fight_in_progress', False))
                    or bool(getattr(self, 'monster_page_turn_active', False))
                    or bool(getattr(self, 'transmute_reveal_active', False))
                    or bool(getattr(self, 'masterquest_resolving', False))
                ):
    ...
    def _mark_scene_refresh(self, scene_key: str) -> None:
        key = str(scene_key or '').strip().lower()
        if not key:
            return
        try:
            self._scene_refresh_timestamps[key] = time.monotonic()
        except Exception:
            pass

    def _scene_refresh_stale(self, scene_key: str, ttl: Optional[float] = None) -> bool:
        key = str(scene_key or '').strip().lower()
        if not key:
            return True
        ttl_seconds = float(ttl if ttl is not None else getattr(self, '_scene_refresh_ttls', {}).get(key, 12.0) or 12.0)
        last_at = float(getattr(self, '_scene_refresh_timestamps', {}).get(key, 0.0) or 0.0)
        if last_at <= 0.0:
            return True
        try:
    ...
    def restore_cloud_slot_backup(self, allow_stored_user: bool = False) -> bool:
        if not SUPABASE_ENABLED:
            return False
        stored = self._stored_cloud_slot_backup()
        stored_user_id = str(stored.get('user_id') or '').strip()
        current_user_id = str(getattr(self, 'auth_user_id', '') or '').strip()
        if allow_stored_user and stored_user_id and not current_user_id:
            self.auth_user_id = stored_user_id
    ...
    def refresh_authenticated_user(self, persist_session: bool = True) -> bool:
        if self.supabase is None:
            ...
            self._clear_auth_reconnect_grace()
            return False
        known_user_id = str(getattr(self, 'auth_user_id', '') or getattr(self, '_auth_reconnect_known_user_id', '') or '')
        known_user_email = str(getattr(self, 'auth_user_email', '') or getattr(self, '_auth_reconnect_known_user_email', '') or '')
        try:
            ...
            was_reconnecting = bool(getattr(self, '_auth_reconnect_started_at', 0.0))
            self._clear_auth_reconnect_grace()
            if was_reconnecting:
    ...
    def load_cloud_slots(self) -> bool:
        ...
        if restored:
            self.refresh_global_season_state()
            self.apply_pending_global_season_reset(False)
            self.set_auth_status(f'Connected, but live cloud load failed. Restored cached chronicle while retrying: {last_exc}', 'warning')
            return True
        self.set_auth_status(f'Connected, but cloud save load failed: {last_exc}', 'warning')
        return False
    ...
    def append_town_communication_message(self, value: str) -> bool:
        ...
        created_at = _chat_iso_now()
        stamp = time.strftime('%H:%M')
        self.town_communications_messages.insert(0, {'author': author, 'body': body, 'stamp': stamp, 'role': 'player', 'created_at': created_at})
        self.town_communications_messages = _recent_chat_messages(self.town_communications_messages, 80)
        self.town_communications_draft = ''
    ...
    def back_to_class_select(self) -> None:
        ...
        if self.active_slot_index is not None and 0 <= self.active_slot_index < len(self.slots):
            self.slots[self.active_slot_index]['current_run_kills'] = 0
...
def ensure_ladder_scene_state(self, new_visit: bool = False, allow_cached: bool = False) -> None:
    ...
    if hasattr(self, '_mark_scene_refresh'):
        self._mark_scene_refresh('ladder')
...
def player_inventory_payload(player: Optional[Player]) -> List[Dict[str, object]]:
    if player is None:
        return []
...
def normalize_public_profile_inventory(raw_items: object) -> List[Item]:
    items: List[Item] = []
...
def public_profile_payload_variants(payload: Dict[str, object]) -> List[Dict[str, object]]:
    if not isinstance(payload, dict):
        return []
    variants: List[Dict[str, object]] = []
    seen_hashes: set[str] = set()

    def add_variant(candidate: Dict[str, object]) -> None:
        cleaned = dict(candidate)
        variant_hash = _stable_payload_hash(cleaned)
    ...
    without_inventory.pop('inventory_items', None)
    add_variant(without_inventory)

    if 'pq_points' in payload:
        legacy_points = dict(payload)
        legacy_points['masterquest_attempts'] = legacy_points.pop('pq_points')
        add_variant(legacy_points)

        legacy_points_without_inventory = dict(legacy_points)
        legacy_points_without_inventory.pop('inventory_items', None)
        add_variant(legacy_points_without_inventory)
    ...
    add_variant({key: payload[key] for key in minimal_keys if key in payload})
    return variants
...
def normalize_public_profile_payload(raw_row: object) -> Dict[str, object]:
    ...
    inventory_items = normalize_public_profile_inventory(raw_row.get('inventory_items'))
...
def load_town_chat_messages(self) -> bool:
    ...
    try:
        cutoff_iso = _chat_cutoff_iso()
        try:
            self.supabase.table('town_chat_messages').delete().lt('created_at', cutoff_iso).execute()
        except Exception:
            pass
        response = (
...
def append_town_communication_message(self, value: str) -> bool:
    ...
    created_at = _chat_iso_now()
    stamp = time.strftime('%H:%M')
    self.town_communications_messages.insert(0, {'id': '', 'author_user_id': str(self.auth_user_id or ''), 'author': author, 'body': body, 'stamp': stamp, 'role': 'player', 'created_at': created_at})
    self.town_communications_messages = _recent_chat_messages(self.town_communications_messages, 80)
    self.town_communications_draft = ''
    self.chat_command_status = ''
    self.town_communications_text = "\n".join(message.get('body', '') for message in self.town_communications_messages[:12])[:4000]
...
def enter_town(self, note: Optional[str] = None) -> None:
    ...
    load_private_messages(self)
    try:
        refresh_guild_hall_state(self, allow_cached=True)
        load_guild_chat_messages(self)
    except Exception as exc:
        self.guild_message_status = f'Guild channel is waiting on database setup: {exc}'
        ...
        for slot_index, raw_slot in enumerate(self.slots, start=1):
            ...
            if profile_payload is not None:
                ...
                if force or self._last_public_profile_hashes.get(mode) != profile_hash:
                    wrote_profile_payload = False
                    for candidate_payload in public_profile_payload_variants(profile_payload):
                        try:
                            self.supabase.table('public_player_profiles').upsert(candidate_payload, on_conflict='user_id,mode').execute()
                            self._last_public_profile_hashes[mode] = _stable_payload_hash(candidate_payload)
                            wrote_profile_payload = True
                            break
                        except Exception as exc:
                            profile_sync_error = str(exc)
                            continue
                    if not wrote_profile_payload:
        ...
        for mode in LADDER_MODE_ORDER:
            ...
            if force or self._last_public_profile_hashes.get(mode) != '__deleted__':
                try:
                    ...
                    pass
                self._last_public_profile_hashes[mode] = '__deleted__'
        self._public_sync_dirty = bool(profile_sync_failed)
        if profile_sync_failed and profile_sync_error:
            self.public_ladder_status = f'Public profile sync is waiting on public_player_profiles setup: {profile_sync_error}'
    except Exception as exc:
        self.public_ladder_status = f'Global ladder sync is waiting on database setup: {exc}'
...
def fetch_public_profile(self, user_id: str = '', character_name: str = '', mode: str = '') -> bool:
        ...
        if not snapshot:
            try:
                ...
                if isinstance(fallback_row, dict) and fallback_row:
                    ...
                    status_message = 'Basic ladder information was found, but this player\'s public equipment snapshot is not readable yet.'
            except Exception:
                pass
    if not snapshot and self.player is not None and cleaned_name and cleaned_name == clean_character_name(self.player.name):
        ...
        snapshot = {
            ...
            'inventory_items': [copy.deepcopy(item) for item in normalize_public_profile_inventory(player_inventory_payload(self.player))],
...
def load_guild_pq_points_by_user(self, user_ids: Optional[set[str]] = None) -> Dict[str, int]:
    cleaned_user_ids = {str(user_id or '').strip() for user_id in (user_ids or set()) if str(user_id or '').strip()}

    def include_user(user_id: str) -> bool:
        return bool(user_id) and (not cleaned_user_ids or user_id in cleaned_user_ids)
    ...
    try:
        query = self.supabase.table('pq_ledger_entries').select('id, user_id').limit(50000)
        if cleaned_user_ids:
            try:
                query = query.in_('user_id', sorted(cleaned_user_ids))
            except Exception:
                pass
        rows = _supabase_response_data(query.execute())
        ...
        if isinstance(rows, list):
            for row in rows:
                ...
                if not include_user(member_user_id):
                    continue
                ...
                pq_by_user[member_user_id] = pq_by_user.get(member_user_id, 0) + 1
    except Exception:
        pass

    fallback_sources = (
        ('public_player_profiles', 'pq_points'),
        ('leaderboard_entries', 'masterquest_attempts'),
    )
    for table_name, points_field in fallback_sources:
        try:
            query = self.supabase.table(table_name).select(f'user_id, {points_field}').limit(50000)
            if cleaned_user_ids:
                try:
                    query = query.in_('user_id', sorted(cleaned_user_ids))
                except Exception:
        ...
        if isinstance(rows, list):
            for row in rows:
                ...
                if not include_user(member_user_id):
                    continue
                try:
                    points = max(0, int(row.get(points_field, 0) or 0))
                except Exception:
                    points = 0
                fallback_totals[member_user_id] = fallback_totals.get(member_user_id, 0) + points
        for member_user_id, total_points in fallback_totals.items():
            pq_by_user[member_user_id] = max(int(pq_by_user.get(member_user_id, 0) or 0), int(total_points or 0))
    return pq_by_user

def refresh_guild_hall_state(self, allow_cached: bool = False, force: bool = False) -> bool:
    if allow_cached and not force and hasattr(self, '_scene_refresh_stale') and not self._scene_refresh_stale('guild_hall'):
        return True
    self.guild_row = {}
    ...
    if not self._guild_cloud_enabled():
        ...
        if hasattr(self, '_mark_scene_refresh'):
            self._mark_scene_refresh('guild_hall')
        return False
    try:
        ...
        if isinstance(membership, dict) and membership.get('guild_id'):
            ...
            if isinstance(member_rows, list) and member_rows:
                member_user_ids = {str(row.get('user_id') or '').strip() for row in member_rows if isinstance(row, dict)}
                pq_points_by_user = self.load_guild_pq_points_by_user(member_user_ids)
                self.guild_member_rows = []
                for row in member_rows:
                    ...
                    self.guild_member_rows.append({
                        ...
                        'pq_points': int(pq_points_by_user.get(member_user_id, 0)),
                    })
...
def refresh_guild_leaderboard(self, force: bool = False, allow_cached: bool = False) -> bool:
    if allow_cached and not force and hasattr(self, '_scene_refresh_stale') and not self._scene_refresh_stale('guild_leaderboard'):
        return True
    self.guild_leaderboard_rows = []
    if not self._guild_cloud_enabled():
        if hasattr(self, '_mark_scene_refresh'):
            self._mark_scene_refresh('guild_leaderboard')
        return False
    try:
        ...
        all_member_user_ids = {str(row.get('user_id') or '').strip() for row in membership_rows if isinstance(row, dict)} if isinstance(membership_rows, list) else set()
        pq_by_user = self.load_guild_pq_points_by_user(all_member_user_ids)
...
def ensure_marketplace_offers(self, force_reroll: bool = False, allow_cached: bool = False) -> None:
    if self.player is None:
        return
    ...
    if (
        ...
        and refresh_level == self.marketplace_offer_refresh_level
        and len(self.marketplace_offers) == 3
        and bool(self.current_marketplace_line)
        and hasattr(self, '_scene_refresh_stale')
        and not self._scene_refresh_stale('marketplace')
    ):
        return
    banter_lines = [
    ...
    if hasattr(self, '_mark_scene_refresh'):
        self._mark_scene_refresh('marketplace')
...
def refresh_bazaar_listings(self, force: bool = False, allow_cached: bool = False) -> None:
    if self.current_slot_mode() == 'SSF':
        ...
        if hasattr(self, '_mark_scene_refresh'):
            self._mark_scene_refresh('bazaar')
        return
    if self.player is None:
        self.bazaar_listings = []
        self.bazaar_last_refresh_at = time.time()
        if hasattr(self, '_mark_scene_refresh'):
            self._mark_scene_refresh('bazaar')
        return
    if allow_cached and not force and hasattr(self, '_scene_refresh_stale') and not self._scene_refresh_stale('bazaar'):
        return
...
SessionState.load_guild_pq_points_by_user = load_guild_pq_points_by_user
...
def main_page(request: Request) -> None:
                ...
                if not snapshot:
                    ...
                    with ui.row().classes('w-full gap-4 mt-5 items-start max-[1000px]:flex-wrap'):
                        ...
                        with ui.card().classes('mq-panel-frame flex-[1.2] min-w-[340px] p-4'):
                            ui.label('INVENTORY ITEMS').classes('mq-panel-caption')
                            if not inventory_items:
                                ui.label('No inventory items were present in the latest public snapshot.').classes('mq-detail-text mt-3')
                            else:
                                with ui.scroll_area().classes('w-full max-h-[420px] pr-2'):
                                    for item in inventory_items:
                                        with ui.card().classes('mq-panel-frame p-3 mt-3'):
                                            ...
                                            ui.html(f"<div class='mq-detail-text mt-2'>{hoverable_item_name_html(item, 'Empty')}</div>")
                                            ui.label(f'{saved_item_type_label(item)} - Tier {item_required_level(item)}').classes('mq-detail-text mt-1')
    ...
    def request_render_refresh(*_args, force: bool = False, delay: float = 0.0) -> None:
        ...
        def _schedule_follow_up(wait_time: float = 0.0) -> None:
            ...
            refresh_state['token'] = int(refresh_state.get('token', 0) or 0) + 1
            token = refresh_state['token']

            def _flush_if_current() -> None:
                if token != refresh_state.get('token'):
                    return
                _flush()
            ...
            if wait_time <= 0.0:
                ...
                ui.timer(wait_time, _flush_if_current, once=True)
            ...
            if state.screen == 'town':
                ...
                with ui.column().classes('mq-town-shell w-full gap-5'):
                    ...
                    with ui.element('div').classes('mq-town-dashboard w-full'):
                        ...
                        with ui.card().classes('mq-log mq-town-comm-card w-full max-w-none p-4 md:p-5'):
            ...
            with ui.column().classes('mq-arena-shell w-full gap-4'):
                ...
                if state.game_tab != 'arena':
                    ...
                    if state.game_tab == 'inventory':
                        ...
                        state.refresh_guild_hall_state(allow_cached=True)
                        ...
                        with ui.column().classes('w-full max-w-[980px] mx-auto gap-3 items-center'):
                            ...
                            with ui.row().classes('gap-2 mt-3 w-full justify-center flex-wrap'):
                                ui.button('Refresh Ladder', on_click=lambda: (state.sync_public_leaderboard(force=True), state.refresh_public_ladder(), state.refresh_public_pq_ledger(), state.refresh_guild_leaderboard(), request_render_refresh())).classes('mq-btn-secondary rounded-xl px-5 py-3 font-semibold')
                            ...
                            with ui.card().classes('mq-card w-full p-6 md:p-7'):
                                ...
                                if not snapshot:
                                    ...
                                    with ui.row().classes('w-full justify-between items-center gap-3 mt-5 flex-wrap'):
                                        ui.label('INVENTORY ITEMS').classes('mq-panel-caption')
                                        ui.label(f'{len(inventory_items)} visible').classes('mq-detail-text')
                                    with ui.card().classes('mq-panel-frame w-full p-4'):
                                        if not inventory_items:
                                            ui.label('No inventory items were present in the latest public snapshot.').classes('mq-detail-text mt-1')
                                        else:
                                            with ui.scroll_area().classes('w-full max-h-[440px] pr-2'):
                                                with ui.row().classes('w-full gap-3 flex-wrap'):
                                                    for item in inventory_items:
                                                        with ui.card().classes('mq-panel-frame p-3 flex-1 min-w-[220px]'):
                                                            ui.html(safe_rarity_badge_html(item))
                                                            ui.html(f"<div class='mq-detail-text mt-2'>{hoverable_item_name_html(item, 'Empty')}</div>")
                                                            ui.label(f'{saved_item_type_label(item)} - Tier {item_required_level(item)}').classes('mq-detail-text mt-1')
                                                            ui.label(item.short_stat_text()).classes('mq-detail-text mt-2')
...
""")
    ...
    def maybe_retry_auth_reconnect() -> None:
        if not getattr(state, '_auth_reconnect_started_at', 0.0):
            return
        was_active = state._auth_reconnect_active()
        state.ensure_auth_session()
        is_active = state._auth_reconnect_active()
        if was_active != is_active or not is_active:
            request_render_refresh()
