...
def enter_town(self, note: Optional[str] = None) -> None:
    ...
    should_refresh_town = True
    try:
        should_refresh_town = self._scene_refresh_stale('town')
    except Exception:
        should_refresh_town = True
    if not should_refresh_town:
        return

    def _refresh_town_support_panels() -> None:
        try:
            load_town_chat_messages(self)
        except Exception:
            pass
        ...
        try:
            ...
            pass
        try:
            self._mark_scene_refresh('town')
        except Exception:
            pass

    try:
        ui.timer(0.05, _refresh_town_support_panels, once=True)
    except Exception:
        _refresh_town_support_panels()
...
def refresh_guild_hall_state(self, allow_cached: bool = False, force: bool = False) -> bool:
    if allow_cached and not force and (self.guild_row or self.guild_invite_rows or self.guild_member_rows) and hasattr(self, '_scene_refresh_stale') and not self._scene_refresh_stale('guild_hall'):
        return True
    self.guild_row = {}
